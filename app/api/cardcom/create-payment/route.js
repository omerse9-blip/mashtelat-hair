...
export async function POST(req) {
  ...
  async function deleteOrders(numbers) {
    if (!numbers.length) return;
    try {
      const { data: rows } = await supabaseAdmin.from("orders").select("id").in("order_number", numbers);
      ...
      console.error("[cardcom][CRITICAL] cleanup itself failed", { numbers, cleanupErr });
    }
  }
  ...
  try {
    ...
    await deleteOrders(orderNumbers);
    return NextResponse.json(
      { error: "מצטערים, אירעה שגיאת שרת, נסו שוב בעוד כמה רגעים" },
      { status: 500 }
    );
  }
}

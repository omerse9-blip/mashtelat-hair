...
const linkStyle = { color: "var(--ink)", textDecoration: "none" };
const mutedLink = { color: "var(--muted)", textDecoration: "none" };

// תווית עמודה: קטנה, מרווחת, אפורה — נסוגה אחורה ומשאירה את התוכן קדמי
const colHeading = {
  fontWeight: 600,
  fontSize: 12,
...
// תוכן אחיד: שחור-רך, אותו קצב אנכי בכל העמודות
const bodyText = {
  color: "var(--ink)",
  fontSize: 15,
  lineHeight: 1.8,
...
function InstagramIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="var(--muted)" aria-hidden="true">
...
function Social(props) {
  ...
  return (
    <a href={props.href} target="_blank" rel="noreferrer" style={wrap}>
      ...
      <span style={{ color: "var(--ink)", fontSize: 15 }}>{props.label}</span>
    </a>
  );
}

  ...
  return (
    <div>
              ...
              {images.map((w) => (
                <figure key={w.id} style={{ margin: 0 }}>
                  <div
                    ...
                    style={{ aspectRatio: "4 / 3", borderRadius: 14, overflow: "hidden", background: "#f4f6f4", cursor: "zoom-in" }}
                  >
                    <img src={w.media_url} alt={w.caption || ""} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  </div>
                  {w.caption && <figcaption style={{ color: "var(--muted)", fontSize: 14, marginTop: 8 }}>{w.caption}</figcaption>}
                </figure>
              ))}
            </div>
          )}
              ...
              {videos.map((w) => (
                <figure key={w.id} style={{ margin: 0 }}>
                  <div style={{ aspectRatio: "16 / 9", borderRadius: 14, overflow: "hidden", background: "#000" }}>
                    <iframe
                      src={`https://www.youtube.com/embed/${w.media_url}`}
                      title={w.caption || "סרטון"}
      ...
      <div style={{ textAlign: "center", marginTop: 56 }}>
          ...
          href={BOOK_URL}
          style={{ display: "inline-block", background: "var(--green)", color: "#fff", fontSize: 17, fontWeight: 700, padding: "14px 36px", borderRadius: 999 }}
        >
          תיאום פגישה
        </a>
      </div>

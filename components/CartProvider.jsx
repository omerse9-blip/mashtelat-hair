...
export function CartProvider({ children }) {
  ...
  // כל הסל עובר לתשלום מקוון רק אם כל הפריטים מקטגוריות שאושרו לכך
  const allOnlinePayable = items.length > 0 && items.every((x) => !!x.onlinePaymentEnabled);
  return (
    <CartContext.Provider value={{ items, addItem, removeItem, setQuantity, clear, count, total, ready, allOnlinePayable }}>
      {children}

...
// מאגר הזרים למנוי, עם כל הגדלים, המחירים והתמונות שלהם (לחישוב מחיר ותמונה לפי גודל שנבחר)
...
// תמונה להצגה עבור זר מסוים בגודל שנבחר - קודם תמונת הגודל המדויק, אחרת כל תמונה אחרת שיש לזר
export function subscriptionFlowerImage(product, sizeKey) {
...
// גרסה מסוננת של getDeliveryOptions, רק לימי חמישי/שישי — לשימוש בעמוד המנוי
export async function getSubscriptionDeliveryDayOptions(daysAhead = 21) {
  const settings = await readDeliveryRows();
  const now = new Date();
  const out = [];
  ...
  for (let i = 0; i < daysAhead; i++) {
    const d = new Date(now);
    d.setDate(now.getDate() + i);
    const dow = d.getDay(); // 4 = חמישי, 5 = שישי
    ...
    out.push({
      key: dow === 4 ? "thursday" : "friday",
      label: dow === 4 ? "חמישי" : "שישי",
    });
  }
...
// חלונות שעה קבועים לימי חמישי/שישי (לפי שעות הפעילות הבסיסיות, לא תאריך ספציפי) — לבחירת שעה קבועה במנוי
export const getSubscriptionWindowOptions = unstable_cache(
  async () => {
    const settings = await readDeliveryRows();
    const dayIndexes = { thursday: 4, friday: 5 };
    const result = {};
    for (const [key, dow] of Object.entries(dayIndexes)) {

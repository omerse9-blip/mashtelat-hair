...
// חריגים עתידיים בשעות הפעילות — להצגת באנר עדכון ללקוחות
// מחזיר רק חריגים שנבדלים מהבסיס של אותו יום בשבוע (שינוי שעה או סגירה)
export async function getDeliveryNotices(daysAhead = 14) {
  ...
  for (const iso of dates) {
    ...
    if (ov.state === "closed") {
      notices.push(`לקוחות יקרים, שימו לב: בתאריך ${dateText} המשתלה תהיה סגורה`);
    } else {
      const closeHHMM = (ov.close || "").slice(0, 5);
      notices.push(`לקוחות יקרים, שימו לב: בתאריך ${dateText} המשתלה תהיה פתוחה עד השעה ${closeHHMM}`);
    }
  }
  return notices;
}

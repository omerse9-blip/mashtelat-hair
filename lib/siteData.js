...
// המחיר הזול ביותר במאגר עבור גודל ספציפי (small/medium/large -> קטן/בינוני/גדול)
...
export async function getSubscriptionDeliveryDayOptions(daysAhead = 21) {
  ...
  // מחזיר את שני הימים הייחודיים בלבד (לא כל תאריך), כי הבחירה היא "חמישי" או "שישי" כקבוע
  const seen = new Set();
  const unique = [];
  for (const o of out) {
    if (!seen.has(o.key)) {

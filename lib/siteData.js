...
// תמונת/סרטון הרקע הראשי הפעיל (Hero) — עם מטמון
export const getActiveHeroImage = unstable_cache(
  async () => {
    ...
    if (error) throw new Error(error.message);
    return data
      ? { url: data.image_url, mediaType: data.media_type || "image" }
      : { url: "/hero-nursery.jpg", mediaType: "image" };
  },
  ["hero-image"],
...
export async function getProductById(id) {
  const { data, error } = await supabase
    .from("products")
    .select("*, product_sizes(*), categories(id, name, kind, disclaimer, size_type, online_payment_enabled)")
    .eq("id", id)
    .eq("is_active", true)
    .single();
...
// מחיר "החל מ-" למנוי הפריחה — הזול ביותר מתוך מאגר הזרים שנבחר לו
export const getSubscriptionMinPrice = unstable_cache(
  async () => {
    const { data: links, error: e1 } = await supabase
      .from("subscription_featured_products")
      .select("product_id");
    if (e1 || !links || !links.length) return null;

    const ids = links.map((l) => l.product_id);
    const { data: products, error: e2 } = await supabase
      .from("products")
      .select("single_price, has_sizes, product_sizes(price)")
      .in("id", ids)
      .eq("is_active", true);
    if (e2 || !products || !products.length) return null;

    let min = null;
    for (const p of products) {
      const price = p.has_sizes && p.product_sizes?.length
        ? Math.min(...p.product_sizes.map((s) => Number(s.price)))
        : (p.single_price != null ? Number(p.single_price) : null);
      if (price != null && !isNaN(price) && (min === null || price < min)) min = price;
    }
    return min;
  },
  ["subscription-min-price"],
  { revalidate: CACHE_SECONDS, tags: ["products", "subscription-featured"] }
);

// מאגר הזרים למנוי, עם כל הגדלים, המחירים והתמונות שלהם
export const getSubscriptionFlowerPool = unstable_cache(
  async () => {
    const { data: links, error: e1 } = await supabase
      .from("subscription_featured_products")
      .select("product_id, position")
    ...
    const { data: products, error: e2 } = await supabase
      .from("products")
      .select("id, name, image_url, single_price, has_sizes, product_sizes(size_label, price, image_url)")
      .in("id", ids)
      .eq("is_active", true);
    if (e2 || !products) return [];
  ...
  ["subscription-flower-pool"],
  { revalidate: CACHE_SECONDS, tags: ["products", "subscription-featured"] }
);

const SIZE_LABEL_BY_KEY = { small: "קטן", medium: "בינוני", large: "גדול" };

// המחיר הזול ביותר במאגר עבור גודל ספציפי - להצגת "החל מ-" בשלב בחירת הגודל
export async function getSubscriptionMinPriceForSize(sizeKey) {
  const pool = await getSubscriptionFlowerPool();
  const label = SIZE_LABEL_BY_KEY[sizeKey];
  let min = null;
  for (const p of pool) {
    if (p.has_sizes && p.product_sizes?.length) {
...
// המחיר הסופי המחייב - הגבוה מבין הזרים הספציפיים שהלקוחה בחרה בפועל (קו מנחה), בגודל שבחרה
export async function getSubscriptionMaxPriceForChosen(sizeKey, chosenProductIds) {
  if (!chosenProductIds || !chosenProductIds.length) return null;
  const pool = await getSubscriptionFlowerPool();
  const label = SIZE_LABEL_BY_KEY[sizeKey];
  let max = null;
  for (const id of chosenProductIds) {
...
// טבלת הנחות המנוי — גודל × תדירות, עם מטמון
export const getSubscriptionDiscounts = unstable_cache(
  async () => {
    const { data, error } = await supabase
      .from("subscription_discounts")
      .select("size, frequency, discount_percent");
    if (error) throw new Error(error.message);
    return data || [];
  },
  ["subscription-discounts"],
  { revalidate: CACHE_SECONDS, tags: ["subscription-discounts"] }
);

export function getDiscountPercent(discounts, size, frequency) {
...
export async function getAddonsForCategory(categoryId) {
  const { data, error } = await supabase
    ...
    .select("product:products(*, product_sizes(*), categories(name, position, size_type, online_payment_enabled))")
    .eq("category_id", categoryId);
  if (error) return [];
...
async function readDeliveryRows() {
  ...
  if (error) return { windows: DEFAULT_WINDOWS, base: {}, overrides: {}, cityFee: 30, hotelFee: 50 };
  const rows = data || [];
  ...
  const cityFee = winRow && winRow.city_fee != null ? Number(winRow.city_fee) : 30;
  const hotelFee = winRow && winRow.hotel_fee != null ? Number(winRow.hotel_fee) : 50;
  ...
  return { windows, base, overrides, cityFee, hotelFee };
}

// דמי המשלוח שהוגדרו (עיר/מלון), לשימוש בעגלה ובצ'קאאוט
export async function getDeliveryFees() {
  const s = await readDeliveryRows();
  return { city: s.cityFee, hotel: s.hotelFee };
}
...
// ימי השבוע האפשריים למנוי — ראשון עד שישי, ללא שבת
const SUBSCRIPTION_DAY_KEYS = {
  0: { key: "sunday", label: "ראשון" },
  1: { key: "monday", label: "שני" },
  2: { key: "tuesday", label: "שלישי" },
  3: { key: "wednesday", label: "רביעי" },
  4: { key: "thursday", label: "חמישי" },
  5: { key: "friday", label: "שישי" },
};

// ימי משלוח אפשריים למנוי — כל יום ראשון-שישי שאינו סגור, בלי שבת בכלל
export async function getSubscriptionDeliveryDayOptions() {
  const settings = await readDeliveryRows();
  const out = [];
  for (let dow = 0; dow <= 5; dow++) {
    const day = settings.base[dow] || { open: "09:00", close: "18:00", state: "open" };
    if (day.state === "closed") continue;
    out.push(SUBSCRIPTION_DAY_KEYS[dow]);
  }
  return out;
}

// חלונות שעה קבועים לכל יום אפשרי במנוי (ראשון-שישי) — לפי שעות הפעילות הבסיסיות
export const getSubscriptionWindowOptions = unstable_cache(
  async () => {
    const settings = await readDeliveryRows();
    const result = {};
    for (let dow = 0; dow <= 5; dow++) {
      const key = SUBSCRIPTION_DAY_KEYS[dow].key;
      const day = settings.base[dow] || { open: "09:00", close: "18:00", state: "open" };
      result[key] = day.state === "closed" ? [] : computeWindows(day, settings.windows, null);
    }
    return result;
  },
  ["subscription-window-options"],
  { revalidate: CACHE_SECONDS, tags: ["delivery-settings"] }
);
...
// =====================================================================
// זמינות עתידית — מוצר שניתן להזמין עכשיו אך נמסר רק ממועד שנקבע
// =====================================================================
const HEB_DAY_NAMES = ["ראשון", "שני", "שלישי", "רביעי", "חמישי", "שישי", "שבת"];

function futureAvailability(product) {
  const iso = product?.available_from_date;
  if (!iso) return null;
  ...
  return { date, d, m, dayName: HEB_DAY_NAMES[date.getDay()], window: product.available_from_window || "" };
}

// תגית קצרה לכרטיס בקטלוג — "זמין מיום שני"
export function availableFromShort(product) {
  const a = futureAvailability(product);
  if (!a) return "";
  return `זמין מיום ${a.dayName}`;
}

// טקסט מלא לעמוד המוצר — "זמין מיום שני 6.10, 12-14"
export function availableFromFull(product) {
  const a = futureAvailability(product);
  if (!a) return "";
  const w = a.window ? `, ${a.window}` : "";
  return `זמין מיום ${a.dayName} ${a.d}.${a.m}${w}`;
}
